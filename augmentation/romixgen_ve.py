...
class VERomixgen:
    ...
    def mix(self, obj_id, bg_id):
        ...
        # txt mix 
        choice = random.randint(0,len(obj_info['sentence'])-1)
        sentence = self.txt_aug([obj_info['sentence'][choice]], [np.random.choice(bg_info['sentence'])])
        
        # label : object 기준 
        label = obj_info['label'][choice]
    ...
    def __call__(self, image_id: str):
        ...
        img, sentence, label = self.mix(obj_id, bg_id)
        return img, sentence, label  
